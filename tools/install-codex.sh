#!/usr/bin/env bash
# Install compiled agent prompts for OpenAI Codex CLI.
#
# Copies flat prompts to ~/.codex/instructions/ and prints
# shell aliases for spawning Codex sessions with --instructions.
#
# Usage:
#   ./tools/install-codex.sh              # install all
#   ./tools/install-codex.sh wf           # install one plugin
#   ./tools/install-codex.sh wf planner   # install one agent

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
COMPILED_DIR="$SCRIPT_DIR/../compiled"
INSTALL_DIR="$HOME/.codex/instructions"
...
for plugin_dir in "$COMPILED_DIR"/*/; do
    ...
    for prompt in "$agents_dir"/*.md; do
        ...
        alias_name="codex-${plugin}-${agent}"
        alias_cmd="alias ${alias_name}='codex --instructions ${INSTALL_DIR}/${plugin}/${agent}.md'"
        aliases+=("$alias_cmd")
    done
done
