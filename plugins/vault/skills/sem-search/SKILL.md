...
description: This skill should be used when the user asks to "search notes",
  "find similar notes", "semantic search", "reindex vault", "rebuild index",
  "query vault", or needs to find notes by meaning rather than keywords.
argument-hint: "<query text> or --reindex"
---
...
- **auto-process**: Uses `qmd vsearch` to confirm filing destinations
- **backlinks**: Uses `qmd vsearch` or `qmd query` to find semantically similar notes
- **find-connections**: Uses `qmd query` for thorough connection discovery
